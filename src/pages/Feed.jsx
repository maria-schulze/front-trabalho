...
const FEED_POSTS = [
  ...
  },
  {
    ...
    rating: 5, 
    likes: 320,
    // Adicionei uma imagem de festa garantida aqui
  ...
  return (
    <div className="w-full bg-[#FFE5E5] flex flex-col font-['Poppins'] min-h-full">
      ...
      <div className="flex justify-center items-center gap-4 pt-8 pb-4 bg-[#FFE5E5] sticky top-0 z-10">
        <button 
            onClick={() => setActiveTab("paravoce")} 
            className={`px-6 py-2 rounded-full font-bold text-sm transition-all duration-300 border-[2px] ${activeTab === "paravoce" ? "border-black text-white shadow-md transform scale-105" : "border-transparent text-gray-500 bg-white/50"}`} 
            style={activeTab === "paravoce" ? gradientBg : {}}
        >
            Para Você
        </button>
        ...
        <button 
            ...
            className={`px-6 py-2 rounded-full font-bold text-sm transition-all duration-300 border-[2px] ${activeTab === "seguindo" ? "border-black text-white shadow-md transform scale-105" : "border-transparent text-gray-500 bg-white/50"}`} 
            style={activeTab === "seguindo" ? gradientBg : {}}
        >
            Seguindo
        </button>
      </div>
      ...
      <div className="flex flex-col gap-6 px-4 pb-4">
          ...
          <div key={post.id} className="flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center overflow-hidden border border-black">
                    <UserIcon />
                </div>
                    ...
                    <div className="flex">
                        {[...Array(5)].map((_, i) => (
                            <div key={i} className="scale-75">
                                <StarIcon percent={i < post.rating ? 100 : 0} size={12} />
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="relative w-full h-64 rounded-2xl overflow-hidden border-2 border-black shadow-sm">
                ...
                {/* Badge de Likes removido conforme solicitado */}
            </div>
        ...
        <div className="p-4 text-gray-400 text-xs text-center">
            Você chegou ao fim do feed por hoje! 🎉
        </div>
      </div>
    </div>
  );
